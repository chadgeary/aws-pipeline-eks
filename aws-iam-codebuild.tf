  ...
  policy = <<EOF
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": [
        "arn:${data.aws_partition.aws-partition.partition}:ec2:${var.aws_region}:${data.aws_caller_identity.aws-account.account_id}:network-interface/*"
      ],
      "Condition": {
        "StringEquals": {
          "ec2:Subnet": [
            "${aws_subnet.aws-private-subnet-A.arn}",
            "${aws_subnet.aws-private-subnet-B.arn}"
          ],
    ...
    },
    {
        ...
        "Resource": ["${aws_s3_bucket.aws-s3-bucket.arn}","${aws_s3_bucket.aws-s3-bucket.arn}/*"]
    },
    {
        "Sid": "CodeKMSCMK",
        "Effect": "Allow",
